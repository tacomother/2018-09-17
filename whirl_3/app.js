...
function Particle() {
  ...
  this.draw = function() {
    ...
    var x =
      (this.loop2.sin * (wh / 3 - 10) + this.loop3.cos * 120) *
      (sharedloop1.sinNorm / 2 + 0.5);
    var y =
      (this.loop2.cos * (wh / 3 - 10) + this.loop3.sin * 120) *
      (sharedloop1.cosNorm / 2 + 0.5);
    var r = 0.2 + 3 * this.loop3.sinNorm * this.loop3.cosNorm; // set the radius
    var c = `hsla(${310 +
      60 *
        (this.loop2.cosNorm + this.offset) *
        this.loop3.sinNorm}, ${100}%, ${50 + 10 * this.loop3.sin}%, ${1})`;
    ...
    for (
      var i = 0;
      i < Math.min(this.history_max * this.loop2.cosNorm, this.history.length);
      i++
    ) {
      ctx.moveTo(tx, ty);
      tx = this.history[i][0];
