...
function Particle() {
  ...
  this.draw = function() {
    ...
    var x =
      this.loop1.sin * (wh / 4 - 10) +
      this.loop2.sin * (wh / 6 - 10) +
      this.loop3.sin * 60;
    var y =
      this.loop1.cos * (wh / 4 - 10) +
      this.loop2.cos * (wh / 6 - 10) +
      this.loop3.cos * 10;
    ...
    for (
      var i = 0;
      i < Math.min(this.history_max * this.loop3.cosNorm, this.history.length);
      i++
    ) {
      ctx.moveTo(tx, ty);
      tx = this.history[i][0];
